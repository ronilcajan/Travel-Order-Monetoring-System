...
class Auth extends CI_Controller
{
    ...
    public function users(){
        ...
        $group = array(1,2,3,4,5); // users that are not authorize to view this page
        if (!$this->ion_auth->in_group($group)){
            // redirect them to the home page because they must be an administrator to view this
            redirect('error_forbidden', 'refresh');
        }
        ...
        if (isset($_POST) && !empty($_POST)) {
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($this->ion_auth->register($identity, $password, $email, $additional_data, $group)) {
                    ...
                    $this->session->set_flashdata('message', 'User account has been created!');
                } else {
        ...
        //list the users
        // $this->data['users'] = $this->ion_auth->join('users_groups', 'users_groups.user_id=users.id')->users()->result();
        ...
        $this->admin->load('admin', 'user/users', $this->data);
    ...
    public function change_profile()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            $this->session->set_flashdata('errors', (validation_errors()) ? validation_errors() : $this->session->flashdata('message'));
        } else {
            if (isset($_POST) && !empty($_POST)) {
                ...
                $update = $this->ion_auth->update($this->session->user_id, $data);
                ...
                if ($update) {
                    ...
                    $this->userModel->updateName($this->session->user_id, $names);
                    $log = array(
                    ...
                    $this->session->set_flashdata('errors', 'Profile has not changed!');
                }
            }
        }
    ...
    public function delete_user($id)
    {
        ...
        {
            // redirect them to the home page because they must be an administrator to view this
            show_error('You must be an administrator to continue this action.');
        } else {
    ...
    public function login()
    {
        ...
        $this->data['info'] = $this->settingsModel->getInfo();
        // validate form input
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                ...
                $this->session->set_flashdata('message', 'Your are successfully logged in!');
                redirect('admin/dashboard', 'refresh');
            } else {
            ...
            $this->auth_layout->load('auth_template', 'auth/login', $this->data);
            // $this->load->('auth' . DIRECTORY_SEPARATOR . 'login', $this->data);
        }
    }
    ...
    public function forgot_password()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $this->auth_layout->load('auth_template', 'auth/forgot_password', $this->data);
        } else {
            $identity_column = $this->config->item('identity', 'ion_auth');
            ...
            if ($forgotten) {
                ...
                $config = [
                    ...
                    'smtp_port' => 465,
                    'smtp_user' => 'ron.system101@gmail.com',
                ...
                $this->email->from($brgy->email,  $sys->powered_b);
                $this->email->to($identity->email);
                $this->email->subject('Forgot Password');
                $body = $this->load->view('email/forgot_password.tpl.php', $data, TRUE);
    ...
    public function reset_password($code = NULL)
    {
        ...
        if ($user) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $this->auth_layout->load('auth_template', 'auth/reset_password', $this->data);
            } else {
